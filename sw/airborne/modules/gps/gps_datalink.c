...
#ifndef EXTERNAL_VISION_SMALL_POS_RES
#define EXTERNAL_VISION_SMALL_POS_RES 1.0
#endif

#ifndef EXTERNAL_VISION_SMALL_SPEED_RES
#define EXTERNAL_VISION_SMALL_SPEED_RES 1.0
#endif
...
/** Parse the full EXTERNAL_VISION message and publish as GPS through ABI */
void gps_datalink_parse_EXTERNAL_VISION(uint8_t *buf)
{
  if (DL_EXTERNAL_VISION_ac_id(buf) != AC_ID) { return; } // not for this aircraft

  uint32_t tow = DL_EXTERNAL_VISION_timestamp(buf);

  struct EnuCoor_f enu_pos, enu_speed;
  enu_pos.x = DL_EXTERNAL_VISION_enu_x(buf);
  enu_pos.y = DL_EXTERNAL_VISION_enu_y(buf);
  enu_pos.z = DL_EXTERNAL_VISION_enu_z(buf);
  enu_speed.x = DL_EXTERNAL_VISION_enu_xd(buf);
  enu_speed.y = DL_EXTERNAL_VISION_enu_yd(buf);
  enu_speed.z = DL_EXTERNAL_VISION_enu_zd(buf);

  struct FloatQuat body_q;
  body_q.qi = DL_EXTERNAL_VISION_body_qi(buf);
  body_q.qx = DL_EXTERNAL_VISION_body_qx(buf);
  body_q.qy = DL_EXTERNAL_VISION_body_qy(buf);
  body_q.qx = DL_EXTERNAL_VISION_body_qz(buf);
...
/** Parse the EXTERNAL_VISION_SMALL message and publish as GPS through ABI */
void gps_datalink_parse_EXTERNAL_VISION_SMALL(uint8_t *buf)
{
  if (DL_EXTERNAL_VISION_SMALL_ac_id(buf) != AC_ID) { return; } // not for this aircraft

  uint32_t tow = DL_EXTERNAL_VISION_SMALL_timestamp(buf);

  struct EnuCoor_i enu_pos_cm, enu_speed_cm;
  struct EnuCoor_f enu_pos, enu_speed;

  /* Convert the 32 bit xyz position to cm seperated */
  uint32_t enu_pos_u = DL_EXTERNAL_VISION_SMALL_enu_pos(buf);
  ...
  enu_pos.x = enu_pos_cm.x / 100.0f * EXTERNAL_VISION_SMALL_POS_RES;
  enu_pos.y = enu_pos_cm.y / 100.0f * EXTERNAL_VISION_SMALL_POS_RES;
  enu_pos.z = enu_pos_cm.z / 100.0f * EXTERNAL_VISION_SMALL_POS_RES;

  /* Convert the 32 bit xyz speed to cm seperated */
  uint32_t enu_speed_u = DL_EXTERNAL_VISION_SMALL_enu_speed(buf);
  ...
  enu_speed.x = enu_speed_cm.x / 100.0f * EXTERNAL_VISION_SMALL_SPEED_RES;
  enu_speed.y = enu_speed_cm.y / 100.0f * EXTERNAL_VISION_SMALL_SPEED_RES;
  enu_speed.z = enu_speed_cm.z / 100.0f * EXTERNAL_VISION_SMALL_SPEED_RES;

  /* Convert the heading with the 1e4 fraction to radians */
  float heading = DL_EXTERNAL_VISION_SMALL_heading(buf) / 1e4;
